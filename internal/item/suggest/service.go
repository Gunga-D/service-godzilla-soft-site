...
func NewService(g getter) *service {
	s := &service{
		g: g,
		indexDesc: suggest.IndexDescription{
			Name:      "items",
...
func (s *service) Suggest(text string) ([]SuggestedItem, error) {
	if s.suggester == nil {
		return []SuggestedItem{}, nil
	}
	...
	res := make([]SuggestedItem, 0, len(suggests))
	for _, suggest := range suggests {
		res = append(res, SuggestedItem{
			Name:        suggest.Value,
